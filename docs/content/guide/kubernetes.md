...
A `head_comment` on an entry whose value is a block, like `spec:` with the
manifest under it, is the key's: it reads, writes and deletes like any
other. A comment above the block's first child stays with that child.
