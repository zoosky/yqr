...
- [ ] `Cargo.toml` pins `noyalib = "0.0.18"`, the adjacent comment names 0.0.18,
      and `Cargo.lock` is refreshed.
- [ ] `bash .github/scripts/local-ci.sh` passes, including `cargo audit`.
- [ ] `tests/fidelity.rs` and `tests/corpus_validation.rs` pass unchanged — the
      bump alters no byte-fidelity behaviour.
- [ ] The §3.2 decision is made, implemented, and recorded in `yqr-f007` §5;
      every `yqr-f007` §5.4 delete test still passes, head-comment and
      keep-chomped cases included.
- [ ] `yqr-f007` §2's "unreleased `feat/v0.0.18`" framing is updated to the
      released API.
- [ ] `yqr-b004` moves to Resolved in `yqr-b000-bug-status.md`, and `yqr-b004`
      §6's adoption notes are marked done.
- [ ] `CHANGELOG.md` records the dependency bump.
...
§3.2's two divergences are worth reporting upstream on the #221 / #222 / #223
precedent: `remove` should fold an entry's contiguous same-indent head comment
and a keep-chomped scalar's kept trailing blank lines into the deletion, as yqr
does. If that lands, §3.2 option (b) becomes clearly correct and `delete.rs`
can genuinely shrink. Not filed yet.
