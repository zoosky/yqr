...
|-----|-------|----------|--------|---------|
| [b010](yqr-b010-reorder-moves-values-not-entries.md) | `swap_items` / `move_item` move values, not entries: every comment stays behind | Medium | Open — noyalib's sequence-reorder mutators exchange the items' **value bytes** and nothing else, so a reorder silently re-attributes every comment in the affected range, at **exit 0**, and `-i` would write it. Both calls pass upstream's integrity guard *by construction*: that guard compares typed values and a comment is not in one. Not a reopened `b004` gap — §2.3's API ships — but a defect inside it, of the `b004` §6.1/§6.4 class, split out of `b004` §6.5 because it is open, unfiled and blocks a staged slice. Three of `swap_items`' documented refusals (flow sequence, multi-line items, differently-indented items) are also stale on 0.0.22. Survives 0.0.23 (§4.1 — the `v0.0.22...main` diff touches `remove` only, and neither reorder call appears in it), so it was filed as its own issue now that the `b004` umbrella `#221` is closed: **noyalib#269**, with the fix offered as **noyalib#270** — which reuses the `owned_entry_range` that `#268` had just added for `remove`, and changes exactly one existing test. **Corrected in flight (§1.1):** upstream's behaviour is deliberate, documented and tested (*"the comment annotates the slot"*), so this is a semantics disagreement, not a defect of the §6.1/§6.4 class; yqr's argument is that `remove` already decides the same question the other way for the same bytes. Awaiting the maintainer's call — closes as Won't Fix rather than Resolved if slot semantics stand. Blocks the `swap`/`move` slice of `yqr-a002` §9 | `yqr-b004`, `yqr-b006`, `yqr-a002`, `yqr-f007`, `yqr-f015`, `yqr-f016` |
...
- Total bugs: 10
- Open: 1 (b010 — noyalib's `swap_items`/`move_item` exchange value bytes
  only, so a reorder silently re-attributes every comment in the range at
  exit 0; blocks the `swap`/`move` slice of `yqr-a002` §9, not yet filed
  upstream)
