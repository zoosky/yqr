...
|-----|-------|----------|--------|---------|
| [b014](yqr-b014-empty-collection-at-the-keys-column-is-invalid-yaml.md) | An empty collection written at its key's own column is invalid YAML that noyalib accepts | Medium | Open (validator half **fixed**) — a block-mapping value must be indented past its key; noyalib's parser accepts one that is not, and its sole-entry `remove` writes one (`on:` / `[]` for the GitHub Actions layout). PyYAML and Ruby's Psych both reject the result. Two faces: the **writer** is upstream's and yqr does not reach it — `yqr-f018` §5 keeps the sole-entry delete in `delete_entry` on the strength of exactly this — while the **validator** face is now **fixed** (2026-08-19): `validate`'s green-tree walk measures each block-mapping entry's value against its key's column and reports `Y103` in default mode, with the two genuinely-valid same-column layouts exempt (a block sequence at its key's column — the GitHub Actions idiom — and a block scalar). Every exemption was checked against PyYAML and Psych rather than reasoned from the spec text. Every guard in the write loop is blind to it for the same reason the `b006`/`b010` blind spots existed: the integrity guard re-parses with the engine whose leniency is the bug. Found by the `yqr-f018` §4 measurement and **filed 2026-08-19 as noyalib#283, with a fix proposed as noyalib#284**, on the argument that upstream's own re-parse guard already refuses the identical removal when the mapping has no following sibling — its own code disagreeing with itself, the shape of argument that carried `b010` and noyalib#280 | `yqr-f018`, `yqr-f016`, `yqr-b006`, `yqr-b010`, `yqr-b011`, `yqr-f012` |
| [b013](yqr-b013-inserted-scalars-take-a-document-wide-quote-style.md) | An inserted scalar is quoted because some unrelated line is quoted | Low | Open — noyalib spells an inserted string with the document's dominant quote style, and the vote counts only quoted scalars against each other, so one `value: "30"` anywhere makes every later insertion quoted however plain its neighbours are. Cosmetic and non-destructive (the value round-trips, `validate` is clean), but it lands at the one line a reader is looking at, and it is what `a001` exists to prevent. Sibling behaviours disagree with it: `set_value` matches the scalar's own style, `rename_key` the neighbouring key's — only insertion reaches document-wide. Found by the `yqr-m003` write tier on its first run and pinned there as it behaves. **Filed 2026-08-19 as noyalib#290**, and — unlike the other three — deliberately *without* a patch: this is a heuristic with a public API attached (`Document::dominant_quote_style` is documented and doctested), so what it counts is the maintainer's call. The filing added two arguments this spec lacked: four plain scalars lose to one quoted one, so the majority never votes; and `set` writes plain at a site whose neighbours are plain while insertion quotes, making the inconsistency internal to noyalib | `yqr-a001`, `yqr-b008`, `yqr-f006`, `yqr-m003` |
| [b012](yqr-b012-insert-refused-when-every-key-holds-a-dot.md) | A new key cannot be inserted into a mapping whose keys all hold a `.` | Medium | Open — `.metadata.labels.tier = "web"` on a Kubernetes manifest is refused, and the message blames a `<<` merge the file does not contain. Upstream's `mapping_insert_anchor` composes each candidate anchor key back into a path **string** and re-parses it, and `parse_query_path` splits on `.` unconditionally, so every key of a dotted mapping is skipped and the only error the function knows about fires. Two defects nested: the `yqr-f007` §6 addressing limit reaching a mapping it was never meant to reach, and a diagnostic asserting a cause instead of describing the observation. Refusal only — exit 5, file untouched. Found by the `yqr-m003` write tier on its first run and **filed 2026-08-19 as noyalib#288, fixed in noyalib#289**: the anchor now comes from the *span tree*, which already holds the target mapping's entries with their spans, so no key is composed back into a path string. The measurement in §3 had missed that the refusal has **two** independent causes upstream — `mapping_insert_anchor` and a duplicate of it inlined in `insert_entry` — and fixing one left the other failing differently. Insert only: `set_value`, `delete`, `rename_key` and the reorder verbs still address through `parse_query_path`, so §3's "all three write paths refuse" is now "two of the three still do". Open until a release carries it | `yqr-f007`, `yqr-a002`, `yqr-b004`, `yqr-m003` |
| [b011](yqr-b011-multiline-flow-collection-fails-to-parse.md) | A multi-line flow collection is valid YAML that yqr cannot read at all | Medium | Open — noyalib refuses to parse a flow collection spread over several lines (`ports: [` / `  80,` / `]`), which libyaml, PyYAML and Psych all accept and no yaml-test-suite case forbids; the message names the right indentation rule and applies it to the closing indicator. Loud rather than silent (exit 5, no damage), but it is a **whole-file read** refusal, so `a001`'s guarantee is vacuous for these files and `validate` calls them unreadable. Found while reviewing the `yqr-f016` §5 flow-delete work — the "flow deletes only work on single-line collections" limitation is a symptom of this. **Filed 2026-08-19 as noyalib#285 and fixed in noyalib#286**, yqr's first contribution to noyalib's *parser* rather than its mutators: the more-indented rule for flow continuation exists because under-indented content is ambiguous with sibling block content, and a `]` or `}` cannot begin block content, so the rule was reaching the terminator too. Under-indented *content* stays refused, which is narrower than libyaml — the ecosystem is more lenient than the spec here, and the fix follows the spec. Open until a release carries it | `yqr-a001`, `yqr-b004`, `yqr-f016` |
...
- Total bugs: 14
- Open: 4 — **all four now filed upstream; three carry a proposed fix, b013
  deliberately does not.**
  - **b014** — an empty collection at its key's own column is invalid YAML
    that noyalib accepts and its sole-entry `remove` writes. The validator
    half is **fixed** as of 2026-08-19 (`Y103`, default mode); the upstream
    writer is filed as noyalib#283 with a fix in noyalib#284.
  - **b013** — an inserted scalar takes the document's dominant quote style,
    which counts only quoted scalars, so one quoted line anywhere decides the
    spelling at every later edit site. Filed as noyalib#290, deliberately
    without a patch: it is a heuristic with a public API attached, so the
    call is the maintainer's.
  - **b012** — a new key cannot be inserted into a mapping whose keys all
    hold a `.`, and the refusal blames a `<<` merge that is not there. Filed
    as noyalib#288 with a fix in noyalib#289. Found with b013 by the
    `yqr-m003` write tier on its first run, and both are pinned there as they
    behave.
  - **b011** — noyalib cannot parse a multi-line flow collection, so yqr
    cannot read the file at all; loud, not silent. Filed as noyalib#285 with
    a fix in noyalib#286.

  The three proposed fixes are all unreleased, so every one of these stays
  open here until a noyalib release carries it; b013 additionally waits on a
  decision about what the vote should count.
