...
|-----|-------|----------|--------|---------|
| [b014](yqr-b014-empty-collection-at-the-keys-column-is-invalid-yaml.md) | An empty collection written at its key's own column is invalid YAML that noyalib accepts | Medium | Open — a block-mapping value must be indented past its key; noyalib's parser accepts one that is not, and its sole-entry `remove` writes one (`on:` / `[]` for the GitHub Actions layout). PyYAML and Ruby's Psych both reject the result. Two faces: the **writer** is upstream's and yqr does not reach it — `yqr-f018` §5 keeps the sole-entry delete in `delete_entry` on the strength of exactly this — while the **validator** face is live, since `yqr validate --strict` walks noyalib's green tree and so calls the file clean, the mirror image of `b011`. Every guard in the write loop is blind to it for the same reason the `b006`/`b010` blind spots existed: the integrity guard re-parses with the engine whose leniency is the bug. Found by the `yqr-f018` §4 measurement; not filed upstream as part of that change | `yqr-f018`, `yqr-f016`, `yqr-b006`, `yqr-b010`, `yqr-b011`, `yqr-f012` |
...
- Open: 4 (b014 — an empty collection at its key's own column is invalid YAML
  that noyalib accepts and its sole-entry `remove` writes; the live half is
  `validate` inheriting the leniency and passing a file the ecosystem cannot
  read; b013 — an inserted scalar takes the document's dominant quote style,
